...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
	char root_dir_buffer[PATH_MAX], test_file_buffer[PATH_MAX];

  if (!realpath(root_dir.c_str(), root_dir_buffer))
    return false;

	// Append the '/' to the end of root_dir_buffer
	int len = strlen(root_dir_buffer);
	root_dir_buffer[len] = '/';
	root_dir_buffer[len + 1] = '\0';

  if (!realpath(test_file.c_str(), test_file_buffer)) {
    return false;
  }
    
  if (strlen(test_file_buffer) <= strlen(root_dir_buffer)) {
    return false;
  }
  
	// Check test_file is a prefix of root_dir
  if (strncmp(root_dir_buffer, test_file_buffer,
  	  strlen(root_dir_buffer)) != 0) {
        return false;
    }
