...
// Checks to see if the linked list contains a key and replaces the old
// key value with the new key value.
static bool KeyIsHere(LLIterator *iter, HTKey_t newkey, HTKeyValue_t *oldkey);
...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t *newChainNode = (HTKeyValue_t*) malloc(sizeof(HTKeyValue_t));
  // Set the chain node's key and value
  newChainNode->key = newkeyvalue.key;
  newChainNode->value = newkeyvalue.value;

  LLIterator *iter = LLIterator_Allocate(chain);
  Verify333(iter != NULL);
  // Verify if the new key is present in this linked list already
  // Replace old key with new one if present
  if (KeyIsHere(iter, newkeyvalue.key, oldkeyvalue)) {
    HTKeyValue_t *currNode;
    LLIterator_Get(iter, (LLPayload_t *) &currNode);
    // Set the node to the new key and value
    currNode->key = newkeyvalue.key;
    currNode->value = newkeyvalue.value;

    free(iter);
    free(newChainNode);
    return true;
  }
  LinkedList_Append(chain, (LLPayload_t) newChainNode);
...
bool HashTable_Find(HashTable *table,
  ...
  // Calculate bucket index and set the chain
  ...
  LLIterator *iter = LLIterator_Allocate(chain);
  Verify333(iter != NULL);
  // Check if key is present in this linked list
  if (KeyIsHere(iter, key, keyvalue)) {
    // Key is present
    free(iter);
    return true;
  }
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // Return false if invalid or empty list
  ...
  // Check if remaining buckets have at least one element
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  // Return false if list is invalid or empty
  if (!HTIterator_IsValid(iter) || iter->ht->num_elements == 0) {
    return false;
  }

  // Get the payload which the iter points to
  HTKeyValue_t *currNode;
  LLIterator_Get(iter->bucket_it, (LLPayload_t *) &currNode);
  // Set the key value
  keyvalue->key = currNode->key;
  keyvalue->value = currNode->value;
...
bool KeyIsHere(LLIterator *iter, HTKey_t new_key, HTKeyValue_t *old_key) {
  HTKeyValue_t *currNode = NULL;
  // Iterate through entire list
  while (LLIterator_IsValid(iter)) {
    // Get the payload
    LLIterator_Get(iter, (LLPayload_t*) &currNode);
    // Replace old key value with new one if found in list
    if (currNode->key == new_key) {
      // Store the new key's data
      old_key->key = currNode->key;
      old_key->value = currNode->value;
      return true;
    }
