...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  int hi_bytes = WriteHeader(f, dt_bytes, mi_bytes);
  if (hi_bytes== kFailedWrite) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
  }
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  int total_bytes = doctable_bytes + memidx_bytes;
  uint8_t next_byte[BUFSIZ];

  for (int i = 0; i < total_bytes; i++) {
    int read_bytes_count = fread(&next_byte, sizeof(uint8_t), BUFSIZ, f);
    for (int j = 0; j < read_bytes_count; j++) {
      crc.FoldByteIntoCRC(next_byte[j]);
    }
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    LinkedList *bucket_list = ht->buckets[i];
		int write_bytes = 0;
    write_bytes = WriteHTBucketRecord(f, record_pos, LinkedList_NumElements(bucket_list), bucket_pos);
    record_pos += write_bytes;
  
    if (LinkedList_NumElements(bucket_list) != 0) {
      write_bytes = WriteHTBucket(f, bucket_pos, bucket_list, fn);
      bucket_pos += write_bytes;
    }
  }
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  BucketRecord bucket_record(num_elts, bucket_offset);
  ...
  fwrite(&bucket_record, sizeof(BucketRecord), 1, f);
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    fseek(f, offset + i * sizeof(record_pos), SEEK_SET); 

    ElementPositionRecord pos_record(element_pos);
    pos_record.ToDiskFormat();

    if (fwrite(&pos_record, sizeof(ElementPositionRecord), 1, f) != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  fwrite(&header, sizeof(DoctableElementHeader), 1, f);
  ...
  fwrite((char * )(kv->value), file_name_bytes, sizeof(char), f);
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  DocIDElementHeader docid_element_header(doc_id, num_positions);
  fseek(f, offset, SEEK_SET);

  docid_element_header.ToDiskFormat();

  fwrite(&docid_element_header, sizeof(DocIDElementHeader), 1, f);
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    position.position = reinterpret_cast<uint64_t> (payload);
  ...
  return sizeof(DocIDElementHeader) + sizeof(DocIDElementPosition) * num_positions;
}
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  WordPostingsHeader word_posting_header(word_bytes, ht_bytes);
  word_posting_header.ToDiskFormat();
  fseek(f, offset, SEEK_SET);

  fwrite(&word_posting_header, sizeof(word_posting_header), 1, f);

  // STEP 18.
  // Write the word itself, excluding the null terminator, in the right
  // place in the file.
  fseek(f, offset + sizeof(WordPostingsHeader), SEEK_SET); 
  fwrite(wp->word, word_bytes, 1, f);
