...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    fread(&curr_header, sizeof(curr_header), 1, file_);
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      // successive positions.
			//
			list<DocPositionOffset_t> list_pos_offset;
      for (int i = 0; i < curr_header.num_positions; i++) {
        DocIDElementPosition pos;
        fread(&pos, sizeof(pos), 1, file_);
        pos.ToHostFormat();
        list_pos_offset.push_back(pos.position);
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file.
		...
		fseek(file_, offset_ + sizeof(header_) + (i * sizeof(BucketRecord)), SEEK_SET);
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      fread(&element_pos, sizeof(element_pos), 1, file_);
      ...
      DocIDElementHeader element;
			fread(&element, sizeof(element), 1, file_);
